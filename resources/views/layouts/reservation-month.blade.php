<div class="calendar-page">
    ...
    <div class="row">
        <div class="col-xs-12 col-sm-4 col-sm-push-4 text-center"><h2><span class="calendar-month">{{date('F Y',$date)}}</span></h2></div>
    ...
    <table class="table calendar">
        ...
        <tr>
            ...
            @foreach ($month->dates as $dates)
                <td data-date="{{$dates->date_time}}" data-php_date="{{$dates->php_date}}" class="{{(isset($dates->reservations) && isset($dates->reservations[0]->name))?'has-event':''}} {{(date('mdY',$dates->php_date) == date('mdY',$current_date))?'active':''}}"><a href="{{url('/admin/reservations/list/'.date('Y/m/d',$dates->php_date))}}">
                    <strong class="day-count">{{$dates->day_count}}</strong>
                    @if(isset($dates->reservations))
                    {{$dates->reservation_count . ' Reservation' . (($dates->reservation_count != 1)?'s':'')}}
